...
struct SheetNavigationView: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            Picker("Drawing space", selection: Binding(get: { space }, set: { space = $0; onSpaceChanged() })) {
                Text("Model").tag(SpaceSelection.model)
                Text("Paper").tag(SpaceSelection.paper)
            }.pickerStyle(.segmented).labelsHidden().frame(width: 130)
            if space == .paper, !sheets.isEmpty {
                Button { onSelect(sheets[index - 1].id) } label: { Image(systemName: "chevron.left") }
                ...
                Button { onSelect(sheets[index + 1].id) } label: { Image(systemName: "chevron.right") }
